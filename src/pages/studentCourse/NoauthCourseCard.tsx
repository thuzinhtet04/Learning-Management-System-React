...
const NoAuthCourseCard = memo(
  ({ enrollments }: { enrollments: CourseResponse , }) => {
    ...
    console.log(Array.isArray(enrollments), 'texting');
  
    const courseData = enrollments?.data?.map(
      ({ instructor_user, ...course }) => ({
        ...course,
              ...
              <div>
                <div className="flex flex-row space-y-3 justify-between items-center">
                  {item.current_price !== item.original_price ? (
                    ...
                    <p className=" text-bold text-2xl">{item.original_price}</p>
                  )}
                </div>
              </div>
            </CardContent>
...
export default NoAuthCourseCard;